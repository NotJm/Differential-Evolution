...
class Differential_Evolution(Algorithm):
    ...
    def _compute_SFS_SIS(self):
        self.SFS = self.population[self.violations == 0]
        self.SIS = self.population[self.violations > 0]
    ...
    def _mutation_operator_(self, idx):
        if not self.resrand_method:
            ...
            return self.bounds_constraints(self.population, self.F, (self.lower, self.upper), idx)
